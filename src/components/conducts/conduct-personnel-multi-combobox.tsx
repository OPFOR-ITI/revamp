...
export function ConductPersonnelMultiCombobox({
  personnel,
  value,
  onChange,
  disabled = false,
}: {
  personnel: ConductPerson[];
  value: string[];
  ...
  const triggerLabel =
    value.length === 0
      ...
      : value.length === 1
        ? (personnel.find((p) => p.personnelKey === value[0])?.name ?? "1 selected")
        : `${value.length} non-participants selected`;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[min(32rem,calc(100vw-2rem))] p-0">
        ...
        >
          <CommandInput placeholder="Search rank, name, or platoon..." />
          <CommandList>
            ...
            {personnel.map((person) => {
              ...
              return (
                <CommandItem
                  ...
                  value={`${person.rank} ${person.name} ${person.platoon}`}
